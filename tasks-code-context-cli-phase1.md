...
- `.ctx/skeleton-prompt.txt` - Embedded default skeleton prompt
...
- [x] 4.0 Command Implementations (init, sync, status, validate, clean, rebuild)
  ...
  - [x] 4.2 In init: Check if `.ctx/` exists, error if already initialized
  - [x] 4.3 In init: Create `.ctx/`, `skeletons/`, write default `config.json` and `skeleton-prompt.txt`
  - [x] 4.4 In init: Add `.ctx/` to `.gitignore` (create if missing)
  ...
  - [x] 4.24 In clean: Scan `.ctx/skeletons/` and remove orphaned files not in index
